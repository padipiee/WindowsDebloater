...
function Set-RegistryKeyValue($registryPath, $targetValue) {
    ...
    if ($currentValue -ne $targetValue) {
        ...
        Write-Host "[0072.03_Disable_loggers_SQMLogger] Registry key $registryPath has been set to $targetValue"
    } else {
        Write-Host "[0072.03_Disable_loggers_SQMLogger] Registry key $registryPath is already set to $targetValue"
    }
}
...
if (Test-Path $registryPath3) {
    ...
    Write-Host "[0072.03_Disable_loggers_SQMLogger] Registry key $registryPath3 does not exist"
}

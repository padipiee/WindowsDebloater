...
function Set-RegistryKeyValue($registryPath, $targetValue) {
    ...
    if ($currentValue -ne $targetValue) {
        ...
        Write-Host "[0072.02_Disable_loggers_CurrentControlSet_AutoLogger-Diagtrack-Listener] Registry key $registryPath has been set to $targetValue"
    } else {
        Write-Host "[0072.02_Disable_loggers_CurrentControlSet_AutoLogger-Diagtrack-Listener] Registry key $registryPath is already set to $targetValue"
    }
}
...
if (Test-Path $registryPath2) {
    ...
    Write-Host "[0072.02_Disable_loggers_CurrentControlSet_AutoLogger-Diagtrack-Listener] Registry key $registryPath2 does not exist"
}

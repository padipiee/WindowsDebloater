...
foreach ($service in $spy_services) {
    ...
    if ($serviceStatus) {
        Write-Host "[0009.02_Delete_services_dmwappushservice] Service found: $($serviceStatus.Name) - $($serviceStatus.DisplayName) - $($serviceStatus.Path)"
        Write-Host "[0009.02_Delete_services_dmwappushservice] Stopping and deleting service $($serviceStatus.Name)..."
        ...
        if ($serviceStatusAfterDeletion) {
            Write-Host "[0009.02_Delete_services_dmwappushservice] Failed to delete service: $($serviceStatusAfterDeletion.Name) - $($serviceStatusAfterDeletion.DisplayName) - $($serviceStatusAfterDeletion.Path)"
        }
        else {
            Write-Host "[0009.02_Delete_services_dmwappushservice] Service deleted successfully: $service"
        }
    }
}

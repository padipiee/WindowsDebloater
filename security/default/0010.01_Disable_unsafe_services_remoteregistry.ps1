...
<#
.SYNOPSIS
This script disables unsafe service RemoteRegistry and stops unneeded services for privacy.
...
.EXAMPLE
.\0010.01_Disable_unsafe_services.ps1
#>


$unsafe_services = "RemoteRegistry"
foreach ($service in $unsafe_services) {
    $serviceStatus = Get-Service -Name $service | Select-Object -ExpandProperty Status
    if ($serviceStatus -eq "Running") {
        Write-Host "[0010.01_Disable_unsafe_services_remoteregistry] Stopping and disabling service: $service"
        Stop-Service -Name $service -Force
        Set-Service -Name $service -StartupType Disabled
    }
    elseif ($serviceStatus -eq "Stopped") {
        Write-Host "[0010.01_Disable_unsafe_services_remoteregistry] Service: $service is already disabled"
    }
    else {
        Write-Host "[0010.01_Disable_unsafe_services_remoteregistry] Service: $service is not found"
    }
}

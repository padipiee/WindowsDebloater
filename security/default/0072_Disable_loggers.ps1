...
$registryPath2 = "HKLM:\SYSTEM\CurrentControlSet\Control\WMI\AutoLogger\AutoLogger-Diagtrack-Listener"
$registryPath3 = "HKLM:\SYSTEM\CurrentControlSet\Control\WMI\AutoLogger\SQMLogger"
...
function Set-RegistryKeyValue($registryPath, $targetValue) {
    ...
    if ($currentValue -ne $targetValue) {
        ...
        Write-Host "[0072_Disable_loggers] Registry key $registryPath has been set to $targetValue"
    } else {
        Write-Host "[0072_Disable_loggers] Registry key $registryPath is already set to $targetValue"
    }
}
...
if (Test-Path $registryPath1) {
    ...
    Write-Host "[0072_Disable_loggers] Registry key $registryPath1 does not exist"
}

if (Test-Path $registryPath2) {
    Set-RegistryKeyValue -registryPath $registryPath2 -targetValue $targetValue
} else {
    Write-Host "[0072_Disable_loggers] Registry key $registryPath2 does not exist"
}

if (Test-Path $registryPath3) {
    Set-RegistryKeyValue -registryPath $registryPath3 -targetValue $targetValue
} else {
    Write-Host "[0072_Disable_loggers] Registry key $registryPath3 does not exist"
}
